...
class SurgeryTypesController extends Controller
{
    ...
    public function index(): Factory|Application|View
    {
        ...
        return view('system.surgerytypes.index', compact('meta'));
    }
    ...
    public function ajaxDatatable(Request $request): JsonResponse
    {
        ...
        $totalRecords = $this->model->query()->withTrashed()
            ...
            ->where('surgery_types.entity_id', session()->get('selected_entity_id'))
            ->orWhere(function ($query) {
                $query->whereNull('surgery_types.entity_id')->whereNull('surgery_types.deleted_at');
            })
            ->count();
        ...
        if (empty($request->get('search')['value'])) {
            ...
            $query  = $this->model->query()->withTrashed()
                ...
                ->whereRaw('LOWER(surgery_types.name) LIKE LOWER(?)', ["%{$search}%"]);
        ...
        if (count($records)) {
            foreach ($records as $record) {
                ...
                $information['category']   = $record->present()->getCategory;
