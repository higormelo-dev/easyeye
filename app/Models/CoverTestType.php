...
class CoverTestType extends Model
{
    use HasUuids;
    use SoftDeletes;
    ...
    {
        static::creating(function (self $additionType) {
            if (blank($additionType->code)) {
                $prefix = $additionType->entity_id ? 'CT' : 'CTP';

                $lastType = static::withoutGlobalScopes()
                    ->when(
                        $additionType->entity_id !== null,
                        fn ($q) => $q->where('entity_id', $additionType->entity_id),
                ...
                $additionType->code = sprintf('%s-%010d', $prefix, $newNumber);
            }
        });
    }
