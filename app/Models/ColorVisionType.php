...
class ColorVisionType extends Model
{
    ...
    {
        static::creating(function (self $colorVisionType) {
            if (blank($colorVisionType->code)) {
                $prefix = $colorVisionType->entity_id ? 'AT' : 'ATP';

                $lastType = static::withoutGlobalScopes()
                    ->when(
                        $colorVisionType->entity_id !== null,
                        fn ($q) => $q->where('entity_id', $colorVisionType->entity_id),
                ...
                $colorVisionType->code = sprintf('%s-%010d', $prefix, $newNumber);
            }
        });
    }
