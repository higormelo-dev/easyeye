...
it('flags a lateral procedure (monocular/binocular) missing eye side as warning', function () {
    $guide  = makeGuide(items: [['description' => 'RETINOGRAFIA MONOCULAR']]);
    $result = (new PreValidateTissGuideService())->validate($guide);

    expect($result->warnings()->pluck('code')->all())->toContain('EYE_SIDE_RECOMMENDED')
        ->and($result->passes())->toBeTrue();
});

it('does not flag a lateral procedure when eye side metadata is already set', function () {
    $guide  = makeGuide(items: [['description' => 'TOMOGRAFIA BINOCULAR', 'metadata' => ['eye_side' => 'AO']]]);
    $result = (new PreValidateTissGuideService())->validate($guide);

    expect($result->all()->pluck('code')->all())->not->toContain('EYE_SIDE_RECOMMENDED');
});

it('does not flag a procedure with no laterality wording', function () {
    $guide  = makeGuide(items: [['description' => 'CONSULTA OFTALMOLOGICA']]);
    $result = (new PreValidateTissGuideService())->validate($guide);
