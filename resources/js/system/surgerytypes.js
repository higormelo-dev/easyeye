...
$(function () {
    ...
    const entityName = trans.actions.surgerytype;

    // Obtém a instância do DataTable já criada pelo Laravel DataTables
    let surgeryTypesDataTable = window.LaravelDataTables['surgerytypes_datatable'];

    // Evento de desenho da tabela - registra os handlers dos botões
    $('#surgerytypes_datatable').on('draw.dt', function () {
        ...
        $('.btn-active').off('click').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    surgeryTypesDataTable.ajax.reload();
                },
                error: handleAjaxError
            });
        });
