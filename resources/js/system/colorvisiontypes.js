...
$(function () {
    ...
    const entityName = trans.actions.colorvisiontype;

    // Obtém a instância do DataTable já criada pelo Laravel DataTables
    let colorVisionTypesDataTable = window.LaravelDataTables['colorvisiontypes_datatable'];

    // Evento de desenho da tabela - registra os handlers dos botões
    $('#colorvisiontypes_datatable').on('draw.dt', function () {
        ...
        $('.btn-edit').off('click').on('click', function () {
            ...
            $.ajax({
                url: `colorvisiontypes/${record_id}/edit`,
                type: 'get',
                headers: {
        ...
        $('.btn-show').off('click').on('click', function () {
            ...
            $("#erro-default").removeClass('show').css('display', 'none');
            $.ajax({
                url: `colorvisiontypes/${record_id}`,
        ...
        $('.btn-active').off('click').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    colorVisionTypesDataTable.ajax.reload();
                },
                error: handleAjaxError
            });
        });
        ...
        $('.btn-restore').on('click', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        method: "get",
                        url: `colorvisiontypes/${record_id}/restore`,
                        dataType: 'json',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
