...
$(function () {
    ...
    let irisTypeDataTable = $('#iristypes_datatable').DataTable({
        "retrieve": true,
        "order": [
            [0, 'desc']
        ],
        ...
        "ajax": {
            'url': '/panel/ajax/datatables/iristypes',
            'dataType': 'json',
            'type': 'POST',
            'data': {
    ...
    irisTypeDataTable.on('draw', function () {
        // Editar
        $('.btn-edit').click(function () {
            record_id = $(this).data('id');
            btn_action = 'update';
            $('.modal-title-default').empty().append('Cadastrar tipo de íris');
        ...
        $('.btn-restore').on('click', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        method: "get",
                        url: `iristypes/${record_id}/restore`,
                        dataType: 'json',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
