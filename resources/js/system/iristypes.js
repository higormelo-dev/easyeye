...
$(function () {
    ...
    const entityName = trans.actions.iristype;

    // Obtém a instância do DataTable já criada pelo Laravel DataTables
    let irisTypesDataTable = window.LaravelDataTables['iristypes_datatable'];

    // Evento de desenho da tabela - registra os handlers dos botões
    $('#iristypes_datatable').on('draw.dt', function () {
        ...
        $('.btn-active').off('click').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    irisTypesDataTable.ajax.reload();
                },
                error: handleAjaxError
            });
        });
