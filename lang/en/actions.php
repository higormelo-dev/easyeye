<?php

declare(strict_types=1);

return [
    'accept'        => 'Accept',
    'action'        => 'Action',
    'actions'       => 'Actions',
    'add'           => 'Add',
    'admin'         => 'Admin',
    'agree'         => 'Agree',
    'archive'       => 'Archive',
    'assign'        => 'Assign',
    'associate'     => 'Associate',
    'attach'        => 'Attach',
    'browse'        => 'Browse',
    'back'          => 'Back',
    'cancel'        => 'Cancel',
    'choose'        => 'Choose',
    'choose_file'   => 'Choose File',
    'choose_image'  => 'Choose Image',
    'click_to_copy' => 'Click to copy',
    'close'         => 'Close',
    'collapse'      => 'Collapse',
    'collapse_all'  => 'Collapse All',
    'comment'       => 'Comment',
    'confirm'       => 'Confirm',
    'connect'       => 'Connect',
    'create'        => 'Create',
    'delete'        => 'Delete',
    'detach'        => 'Detach',
    'details'       => 'Details',
    'disable'       => 'Disable',
    'discard'       => 'Discard',
    'done'          => 'Done',
    'down'          => 'Down',
    'duplicate'     => 'Duplicate',
    'edit'          => 'Edit',
    'enable'        => 'Enable',
    'expand'        => 'Expand',
    'expand_all'    => 'Expand All',
    'explanation'   => 'Explanation',
    'export'        => 'Export',
    'file'          => 'File',
    'files'         => 'Files',
    'go_home'       => 'Go Home',
    'hide'          => 'Hide',
    'home'          => 'Home',
    'image'         => 'Image',
    'impersonation' => 'Impersonating as',
    'impersonate'   => [
        'use_as'         => 'Act as this user',
        'exit'           => 'Back to my account',
        'started'        => 'You are now acting as :name.',
        'ended'          => 'Session ended. You are back to your original account.',
        'already_active' => 'An impersonation session is already active. End it before starting a new one.',
        'only_clients'   => 'You can only impersonate users from client entities.',
        'user_inactive'  => 'Cannot impersonate an inactive user.',
        'banner_prefix'  => ':name is acting as',
        'banner_in'      => 'in',
    ],
    'import'                  => 'Import',
    'introduction'            => 'Introduction',
    'like'                    => 'Like',
    'load'                    => 'Load',
    'localize'                => 'Localize',
    'log_in'                  => 'Log In',
    'log_out'                 => 'Log Out',
    'language'                => 'Language',
    'language_changed'        => 'Language changed successfully.',
    'entity_language_changed' => 'Default company language changed successfully.',
    'language_reset'          => 'Language preference removed. Using company language.',
    'use_entity_language'     => 'Use company language',
    'named'                   => [
        'add'       => 'Add :name',
        'choose'    => 'Choose :name',
        'create'    => 'Create :name',
        'delete'    => 'Delete :name',
        'duplicate' => 'Duplicate :name',
        'edit'      => 'Edit :name',
        'export'    => 'Export :name',
        'hide'      => 'Hide :name',
        'import'    => 'Import :name',
        'new'       => 'New :name',
        'restore'   => 'Restore :name',
        'save'      => 'Save :name',
        'search'    => 'Search :name',
        'show'      => 'Show :name',
        'update'    => 'Update :name',
        'view'      => 'View :name',
    ],
    'new'             => 'New',
    'no'              => 'No',
    'open'            => 'Open',
    'open_website'    => 'Open on the website',
    'preview'         => 'Preview',
    'price'           => 'Price',
    'record'          => 'Record',
    'records'         => 'Records',
    'restore'         => 'Restore',
    'save'            => 'Save',
    'save_and_close'  => 'Save & Close',
    'save_and_return' => 'Save & Return',
    'search'          => 'Search',
    'select'          => 'Select',
    'select_all'      => 'Select All',
    'send'            => 'Send',
    'settings'        => 'Settings',
    'show'            => 'Show',
    'show_all'        => 'Show All',
    'sign_in'         => 'Sign In',
    'solve'           => 'Solve',
    'start'           => 'Start',
    'stop'            => 'Stop',
    'submit'          => 'Submit',
    'subscribe'       => 'Subscribe',
    'switch'          => 'Switch',
    'switch_to_role'  => 'Switch To Role',
    'tag'             => 'Tag',
    'tags'            => 'Tags',
    'target_link'     => [
        'blank'  => 'Open in a new window',
        'parent' => 'Open in a parent frame',
        'self'   => 'Open in a current window',
        'top'    => 'Open in the topmost frame',
    ],
    'translate'              => 'Translate',
    'translate_it'           => 'Translate It',
    'unpack'                 => 'Unpack',
    'unsubscribe'            => 'Unsubscribe',
    'up'                     => 'Up',
    'update'                 => 'Update',
    'user'                   => 'User',
    'users'                  => 'Users',
    'view'                   => 'View',
    'yes'                    => 'Yes',
    'password'               => 'Password',
    'dont_have_account'      => 'Don\'t have an account?',
    'new_account'            => 'Create a new account',
    'email'                  => 'Email Address',
    'edit_profile'           => 'Edit Profile',
    'created_at'             => 'Created At',
    'updated_at'             => 'Updated At',
    'deleted_at'             => 'Deleted At',
    'active'                 => 'Active',
    'inactive'               => 'Inactive',
    'subdomain'              => 'Subdomain',
    'national_registration'  => 'National Registration',
    'state_registration'     => 'State Registration',
    'municipal_registration' => 'Municipal Registration',
    'website'                => 'Website',
    'billing_cycle'          => 'Billing Cycle',
    'sort_order'             => 'Order',
    'starts_at'              => 'Starts At',
    'ends_at'                => 'Ends At',
    'trial_ends_at'          => 'Trial Until',
    'grace_period_ends_at'   => 'Grace Period Until',
    'cancelled_at'           => 'Cancelled At',
    'subscription'           => 'Subscription',
    'plan'                   => 'Plan',
    'status'                 => 'Status',
    'features'               => 'Features',
    'unlimited'              => 'Unlimited',
    'type'                   => 'Type',
    'deleted_record'         => 'Deleted Record (Inactive)',
    'serial_number'          => 'Serial Number',
    'integrators'            => 'Integrators',
    'equipments'             => 'Equipments',
    'entity'                 => 'Entity',
    'integrator'             => 'Integrator',
    'user_integrator'        => 'User Integrator',
    'user_integrators'       => 'User Integrators',
    'doctor'                 => 'Doctor',
    'patient'                => 'Patient',
    'patients'               => 'Patients',
    'record_advicen'         => 'Record Advicen',
    'record_specialty'       => 'Record Specialty',
    'code'                   => 'Code',
    'cellphone'              => 'Cellphone',
    'gender'                 => 'Gender',
    'male'                   => 'Male',
    'female'                 => 'Female',
    'covenant'               => 'Covenant',
    'table'                  => 'Table',
    'color'                  => 'Color',
    'visittype'              => 'Visit Type',
    'visit_type'             => 'Appointment Type',
    'date_time'              => 'Date / Time',
    'situation'              => 'Status',
    'arrived_at'             => 'Arrival',
    'schedule'               => 'Schedule',
    'scheduling'             => 'Schedules',
    'arrived'                => 'Arrived',
    'mark_arrival'           => 'Mark Arrival',
    'patient_record'         => 'Patient Record',
    'register_record'        => 'Register Record',
    'skin'                   => 'Skin',
    'skintype'               => 'Skin Type',
    'iris'                   => 'Iris',
    'iristype'               => 'Iris Type',
    'surgerytype'            => 'Surgery Type',
    'covertesttype'          => 'Cover Test Type',
    'colorvisiontype'        => 'Chromatic Vision Type',
    'additiontype'           => 'Addition Type',
    'visualacuitytype'       => 'Visual Acuity Type',
    'lense'                  => 'Lense',
    'nearpointconvergence'   => 'Near Point Convergence',
    'category'               => 'Category',
    'default_procedure'      => 'Default procedure',
    'version'                => 'Version',
    'abbreviation'           => 'Abbreviation',
    'scale'                  => 'Scale',
    'covenant_table_hint'    => 'Check if this insurance has its own billing table.',
    'name'                   => 'Name',
    'rule'                   => 'Role',
    'away'                   => 'Away',
    'years'                  => 'years',
    'loading'                => 'Loading',
    'not_informed'           => 'Not informed',
    'no_records'             => 'No records found',
    'records_found'          => 'records found',
    'total'                  => 'Total',
    'sort_by'                => 'Sort by',
    'recent'                 => 'Recent',
    'oldest'                 => 'Oldest',
    'view_mode'              => 'View mode',
    'table_view'             => 'Table view',
    'card_view'              => 'Card view',
    'near'                   => 'Near',
    'sidemenu'               => [
        'dashboard'                => 'Dashboard',
        'doctors'                  => 'Doctors',
        'patients'                 => 'Patients',
        'entities'                 => 'Entities',
        'finance'                  => 'Finance',
        'settings'                 => 'Settings',
        'access_control'           => 'Access Control',
        'covenants'                => 'Covenants',
        'skintypes'                => 'Skin Types',
        'iristypes'                => 'Iris Types',
        'visittypes'               => 'Appointment Types',
        'surgerytypes'             => 'Surgery Types',
        'covertesttypes'           => 'Cover Test Types',
        'colorvisiontypes'         => 'Chromatic Vision Types',
        'additiontypes'            => 'Addition Types',
        'visualacuitytypes'        => 'Visual Acuity Types',
        'schedules'                => 'Schedules',
        'schedules_calendar'       => 'Calendar',
        'lenses'                   => 'Lenses',
        'iol_lenses'               => 'Cataract Lenses',
        'nearpointconvergences'    => 'Near Point Convergences',
        'medical_records'          => 'Medical Records',
        'reports'                  => 'Reports',
        'resources'                => 'Resources',
        'partners'                 => 'Partners',
        'ai_providers'             => 'AI Providers',
        'ai_credit_purchases'      => 'AI credit purchases',
        'financial'                => 'Financial',
        'cash_flow'                => 'Cash Flow',
        'tiss_billing'             => 'TISS Billing',
        'financial_reports'        => 'Financial Reports',
        'management_dashboard'     => 'Management Dashboard',
        'tiss_glosas'              => 'TISS Claims',
        'report_cash_flow'         => 'Cash Flow Report',
        'report_billing'           => 'Billing Report',
        'payment_gateways'         => 'Payment Gateways',
        'plans'                    => 'Plans',
        'subscriptions'            => 'Subscriptions',
        'gateways'                 => 'Gateways',
        'users'                    => 'Users',
        'ai_assistant'             => 'AI Assistant',
        'ai_usage'                 => 'Usage & dashboard',
        'ai_prompts'               => 'My prompts',
        'roles'                    => 'Roles & Permissions',
        'clinic_resources'         => 'Rooms / Units',
        'ophthalmology_parameters' => 'Ophthalmology Parameters',
        'settings_clinical'        => 'Clinic',
        'settings_attendance'      => 'Appointments',
        'settings_users'           => 'Users & Permissions',
        'settings_documents'       => 'Documents',
        'settings_ophthalmology'   => 'Ophthalmology',
    ],

    'reports' => [
        'schedules'         => 'Production Report',
        'schedules_label'   => 'Production',
        'absenteeism'       => 'Absenteeism Report',
        'absenteeism_label' => 'Absenteeism',
        'cash_flow'         => 'Cash Flow',
        'cash_flow_label'   => 'Cash Flow',
        'covenants'         => 'Covenant Billing',
        'covenants_label'   => 'Covenant Billing',
    ],

    // Weekdays
    'weekdays' => [
        0 => 'Sunday',
        1 => 'Monday',
        2 => 'Tuesday',
        3 => 'Wednesday',
        4 => 'Thursday',
        5 => 'Friday',
        6 => 'Saturday',
    ],

    // Work schedule / blocks
    'work_schedule'                 => 'Work Schedule',
    'work_schedule_interval'        => 'Appointment interval',
    'work_schedule_interval_hint'   => '(clinic default: :value min)',
    'work_schedule_add_range'       => 'Add time slot',
    'work_schedule_time_from'       => 'From',
    'work_schedule_time_until'      => 'to',
    'work_schedule_day_off'         => 'Not available on this day.',
    'work_schedule_blocks'          => 'Blocks / Absences',
    'work_schedule_no_blocks'       => 'No upcoming blocks.',
    'work_schedule_duplicate_range' => 'Two ranges start at the same time on the same day. Adjust the times and save again.',
    'work_schedule_saved'           => 'Schedule saved successfully.',
    'work_schedule_save'            => 'Save Schedule',
    'work_schedule_load_error'      => 'Error loading data.',
    'work_schedule_save_error'      => 'Error saving schedule.',
    'block_add'                     => 'Add Block',
    'block_reason_placeholder'      => 'Reason (optional)',
    'block_type_absence'            => 'Absence',
    'block_type_holiday'            => 'Holiday',
    'block_type_meeting'            => 'Meeting / Appointment',
    'block_type_other'              => 'Other',
    'block_created'                 => 'Block created successfully.',
    'block_deleted'                 => 'Block removed.',
    'resource_schedule_saved'       => 'Availability saved successfully.',
    'work_schedule_outside'         => 'Time outside the doctor\'s work schedule (:day at :time).',
    'work_schedule_interval_error'  => 'Time does not correspond to a valid slot. The doctor\'s interval is :interval minutes.',
    'work_schedule_blocked'         => 'Doctor is blocked at this time (absence, holiday or scheduled commitment).',
    'resource_double_booked'        => 'Resource ":name" is already occupied at this time.',
    'resource_unavailable'          => 'Resource ":name" is not available (:day at :time).',
    'resource_blocked'              => 'Resource ":name" is blocked at this time.',

    // Schedule situations
    'situation_scheduled'  => 'Scheduled',
    'situation_confirmed'  => 'Confirmed',
    'situation_waiting'    => 'Waiting',
    'situation_dilating'   => 'Dilating',
    'situation_exam'       => 'In Exam',
    'situation_inprogress' => 'In Consultation',
    'situation_attended'   => 'Attended',
    'situation_noshow'     => 'No Show',
    'situation_cancelled'  => 'Cancelled',

    // Patient mood
    'mood_happy'   => 'Calm',
    'mood_anxious' => 'Anxious',
    'mood_nervous' => 'Agitated',
    'mood_angry'   => 'Irritated',

    // Schedule attendance type
    'attendance_type_consultation'     => 'Consultation',
    'attendance_type_return'           => 'Follow-up',
    'attendance_type_urgency'          => 'Urgent',
    'attendance_type_preop'            => 'Pre-op evaluation',
    'attendance_type_postop'           => 'Post-op evaluation',
    'attendance_type_second_opinion'   => 'Second opinion',
    'attendance_type_teleconsultation' => 'Teleconsultation',

    // Schedule specialty / area
    'specialty_general'         => 'General ophthalmology',
    'specialty_cataract'        => 'Cataract',
    'specialty_glaucoma'        => 'Glaucoma',
    'specialty_retina_vitreous' => 'Retina and vitreous',
    'specialty_cornea'          => 'Cornea',
    'specialty_pediatric'       => 'Pediatric ophthalmology',
    'specialty_strabismus'      => 'Strabismus',
    'specialty_neuro'           => 'Neuro-ophthalmology',
    'specialty_uveitis'         => 'Uveitis',
    'specialty_oculoplastic'    => 'Oculoplastic surgery',
    'specialty_refractive'      => 'Refractive surgery',
    'specialty_contact_lenses'  => 'Contact lenses',
    'specialty_low_vision'      => 'Low vision',

    // Schedule card actions
    'change_situation'      => 'Change status',
    'patient_mood_label'    => 'Patient mood',
    'more_options'          => 'More options',
    'reschedule_action'     => 'Reschedule',
    'clear'                 => 'Clear',
    'reschedule_success'    => 'Reschedule completed successfully.',
    'reschedule_error_date' => 'Date and time are required.',

    // Age
    'age' => [
        'label'        => 'Age',
        'not_informed' => 'NOT INFORMED',
        'year'         => '{1} 1 YEAR|[2,*] :count YEARS',
        'month'        => '{1} 1 MONTH|[2,*] :count MONTHS',
        'day'          => '[0,1] 1 DAY|[2,*] :count DAYS',
    ],

    // Marital status
    'marital_status' => [
        1 => 'Single',
        2 => 'Married',
        3 => 'Divorced',
        4 => 'Widowed',
        5 => 'Stable Union',
        6 => 'Legally Separated',
        7 => 'Marital',
        8 => 'Other',
    ],

    'messages' => [
        'register'              => 'Register :name',
        'edit'                  => 'Edit :name',
        'view'                  => 'View :name',
        'delete_confirm_title'  => 'Delete?',
        'delete_confirm_text'   => 'Are you sure you want to delete this record? This action cannot be undone.',
        'restore_confirm_title' => 'Restore?',
        'restore_confirm_text'  => 'Are you sure you want to restore this record? This action cannot be undone.',
        'confirm_yes'           => 'Yes',
        'confirm_no'            => 'No',
    ],

    'medical_records' => [
        'imaging_error'        => 'Could not load the exams. Try again.',
        'imaging_empty'        => 'No imaging exams registered for this patient.',
        'imaging_exam'         => 'Exam',
        'imaging_exams_short'  => 'Imaging',
        'imaging_exams_short2' => 'exams',
        'imaging_loading'      => 'Loading exams…',
        'imaging_no_preview'   => 'No preview — use the original file.',
        'imaging_open_module'  => 'Open imaging module',
        'imaging_original'     => 'Original',
        'imaging_title'        => 'Patient imaging exams',
        'create_hint_after'    => 'to keep the visit open — or click an action on the bar directly (prescription, certificate, report): it saves and opens right after. The visit only ends on Finish.',
        'create_hint_before'   => 'Fill in at least the',
        'create_hint_middle'   => 'and click',
        'flow_continue'        => 'Continue visit',
        'flow_continue_hint'   => 'Leave the screen keeping "In progress" — nothing is finished.',
        'flow_dilate'          => 'Dilate',
        'flow_dilate_hint'     => 'Status becomes "Dilating" and the record stays open for when the patient returns.',
        'flow_exam'            => 'Perform exam',
        'flow_exam_hint'       => 'Status becomes "In exam" and the record stays open for when the patient returns.',
        'flow_finish'          => 'Finish visit',
        'flow_finish_hint'     => 'Visit completed — status becomes "Attended".',
        'flow_subtitle'        => 'Closing the record does not finish the visit — choose where the patient goes on the Schedule.',
        'flow_title'           => 'What happens with the patient now?',
        // Record bottom bar — Save ≠ Finish (2026-09-02)
        'flow_save'           => 'Save',
        'flow_save_hint'      => 'Saves what was filled so far and keeps the visit open.',
        'flow_bar_label'      => 'Visit progress',
        'flow_done_finish'    => 'Visit of :name finished — status "Attended" on the Schedule.',
        'flow_done_dilate'    => ':name is dilating — the record stays open for when they return.',
        'flow_done_exam'      => ':name is in exam — the record stays open for when they return.',
        'back_to_schedule'    => 'Back to Schedule',
        'flow_cash_blocked'   => 'Record saved. To finish, the front desk must register the cash entry for this visit — the visit stays open.',
        'flow_already_closed' => 'Record saved, but the visit is already ":status" on the Schedule — status was not changed.',
        // Post-Save prompt: where does the doctor want to go now?
        'save_prompt_title'         => 'Record saved!',
        'save_prompt_subtitle'      => 'What do you want to do now?',
        'save_prompt_continue'      => 'Continue on the record',
        'save_prompt_continue_hint' => 'Stay on this screen — the visit is still in progress.',
        'save_prompt_agenda'        => 'Go to the Schedule',
        'save_prompt_agenda_hint'   => 'Saved — the visit stays in progress, you move on to the next patient.',
        'save_prompt_list'          => 'View patient records',
        'save_prompt_list_hint'     => 'Full visit history for this patient.',
        'schedule'                  => 'Schedule',
        // ── Cobertura i18n do prontuário (29/08/2026) ────────────────────
        'ai_insert_confirm'       => 'This field already has content. Append the AI suggestion at the end?',
        'collapse_panel'          => 'Collapse panel',
        'conduct'                 => 'Conduct',
        'customize'               => 'Customize',
        'customize_hint'          => 'Choose which sections appear and their order in each column. Hidden sections keep their data.',
        'customize_title'         => 'Customize my record',
        'default_mode_hint'       => 'New visits will open in this format',
        'default_mode_saved'      => 'Set as your default!',
        'diagnoses'               => 'Diagnoses',
        'empty_record'            => 'No data filled in this record.',
        'evolution'               => 'Evolution',
        'evolution_history'       => 'Evolution history',
        'evolution_new'           => 'New evolution',
        'evolution_ph'            => 'Describe the patient\'s clinical evolution...',
        'evolution_save'          => 'Save evolution',
        'evolution_save_error'    => 'Could not save the evolution.',
        'expand_panel'            => 'Expand panel',
        'free_template_apply'     => 'Use my template',
        'free_template_overwrite' => 'The box already has content. Replace with your template?',
        'free_template_save'      => 'Save as my free-text template',
        'free_template_saved'     => 'Template saved!',
        'free_text_hint'          => 'Stored in the record\'s Observations field — data stays structured.',
        'free_text_label'         => 'Evolution / visit (free text)',
        'free_text_ph'            => 'Describe the visit freely...',
        'group_anamnesis'         => 'Anamnesis / History',
        'group_anamnesis_hint'    => 'start of the visit',
        'group_conclusion'        => 'Visit conclusion',
        'group_conclusion_hint'   => 'diagnosis and wrap-up',
        'is_default_mode'         => 'Your default',
        'layout_saved'            => 'Template saved!',
        'left_column'             => 'Left column',
        'lenses_obs_ph'           => 'Continuous use, guidance, lens preference…',
        'lenses_obs_short'        => 'NOTES',
        'loading'                 => 'Loading…',
        'mode_custom'             => 'My record',
        'mode_default'            => 'EasyEye default',
        'mode_free'               => 'Free text',
        'new'                     => 'New',
        'no_evolutions'           => 'No evolutions recorded for this patient.',
        'open_full'               => 'Open full record',

        // Structured procedure execution (Phase 3 gap-fill — stock ↔ record)
        'procedures'                   => 'Procedures',
        'procedures_history'           => 'Procedure history',
        'procedures_new'               => 'Request procedure',
        'procedures_empty'             => 'No procedures recorded for this patient.',
        'procedures_loading'           => 'Loading procedures…',
        'procedures_error'             => 'Could not load procedures.',
        'procedures_search_ph'         => 'Search catalog procedure...',
        'procedures_eye_label'         => 'Eye',
        'procedures_type_label'        => 'Type',
        'procedures_notes_label'       => 'Notes',
        'procedures_request'           => 'Request',
        'procedures_request_error'     => 'Could not request the procedure.',
        'procedures_select_first'      => 'Select a procedure from the catalog.',
        'procedures_mark_done'         => 'Confirm execution',
        'procedures_cancel'            => 'Cancel',
        'procedures_cancel_confirm'    => 'Cancel this procedure request?',
        'procedures_cancel_error'      => 'Could not cancel the procedure.',
        'procedures_consumption_title' => 'Material consumption (optional)',
        'procedures_consumption_hint'  => 'Suggestion based on this procedure\'s registered bill of materials — adjust quantities or remove what was not used.',
        'procedures_no_bom'            => 'No material suggestion registered.',
        'procedures_add_item'          => 'Add item',
        'procedures_remove_item'       => 'Remove',
        'procedures_lot_label'         => 'Lot',
        'procedures_lot_none'          => 'No lot with available balance',
        'procedures_qty_label'         => 'Quantity',
        'procedures_done_error'        => 'Could not confirm execution.',
        'procedures_done_success'      => 'Procedure executed — stock updated.',
        'procedures_module_disabled'   => 'Stock module not included in the plan — execution without material consumption.',
        'previous_records'             => 'Previous visits',
        'reload'                       => 'Reload',
        'right_column'                 => 'Right column',
        'save_my_layout'               => 'Save as my template',
        'set_default_mode'             => 'Set as my default',
        'signed'                       => 'Signed',
        'view_full'                    => 'View full record',
        'view_title'                   => 'Medical record',

        'title'        => 'Medical Record',
        'create'       => 'New Medical Record',
        'edit'         => 'Edit Medical Record',
        'save'         => 'Save Medical Record',
        'saved'        => 'Medical record saved successfully.',
        'locked'       => 'Signed / Locked',
        'loading_more' => 'Loading more records…',
        'no_records'   => 'No medical records found.',
        'end_of_list'  => 'All records loaded.',

        // Tabs
        'tab_anamnesis'     => 'Anamnesis',
        'tab_exam'          => 'Physical Exam',
        'tab_refraction'    => 'Refraction',
        'tab_findings'      => 'Findings',
        'tab_diagnosis'     => 'Diagnosis',
        'tab_documetations' => 'Documents',
        'tab_files'         => 'Files',

        // Anamnesis
        'doctor'                  => 'Attending physician',
        'complaint'               => 'Main complaint',
        'complaint_ph'            => 'Describe the patient\'s main complaint',
        'hda'                     => 'HPI (History of Present Illness)',
        'hda_ph'                  => 'Describe the history of present illness',
        'self'                    => 'Self',
        'family'                  => 'Family',
        'diabetic'                => 'Diabetic',
        'diabetic_family'         => 'Family diabetic',
        'hypertensive'            => 'Hypertensive',
        'hypertensive_family'     => 'Family hypertensive',
        'glaucomatous'            => 'Glaucomatous',
        'glaucomatous_family'     => 'Family glaucomatous',
        'others'                  => 'Others',
        'others_history_ph'       => 'Describe other relevant history…',
        'ocular_surgical_history' => 'Ocular surgical history',
        'medications_in_use'      => 'Current medications',
        'history'                 => 'History',
        'clinical_history'        => 'Clinical History',

        // Physical Exam
        'chromatic_vision'          => 'Chromatic Vision',
        'near_point'                => 'NPC',
        'cover_test'                => 'Cover Test',
        'av_without'                => 'VA without correction',
        'av_with'                   => 'VA with correction',
        'ocular_motility'           => 'Ocular motility',
        'tonometry'                 => 'Tonometry',
        'tonometry_h'               => 'Time',
        'current_time'              => 'Current time',
        'print_tonometry'           => 'Print Tonometry Report',
        'doctor_required_for_print' => 'Please select the responsible doctor before printing.',
        'pachymetry'                => 'Pachymetry',
        'gonioscopy'                => 'Gonioscopy',

        // Refraction
        'dynamic'                => 'Dynamic',
        'static'                 => 'Static',
        'spherical'              => 'Spherical',
        'cylindrical'            => 'Cylindrical',
        'axis'                   => 'Axis',
        'addition'               => 'Addition',
        'presbyopia_addition'    => 'Presbyopia addition',
        'presbyopia_obs_title'   => 'Calculate Presbyopia',
        'presbyopia_obs_label'   => 'Lens observation (optional)',
        'presbyopia_obs_ph'      => 'Enter the lens observation before calculating…',
        'presbyopia_obs_confirm' => 'Calculate',
        'calc'                   => 'Calculate',
        'lens_away'              => 'Far',
        'lens_near'              => 'Near',
        'od'                     => 'OD',
        'oe'                     => 'OS',

        // Findings
        'biomicroscopy'    => 'Biomicroscopy',
        'biomicroscopy_ph' => 'Biomicroscopic exam within normal limits',
        'fundoscopy'       => 'Fundoscopy',
        'fundoscopy_ph'    => 'Fundoscopic exam within normal limits',
        'general_obs'      => 'General observation',
        'lenses_obs'       => 'Lenses observation',

        // Diagnosis
        'cid10'                    => 'ICD-10',
        'diagnosis_description'    => 'Diagnosis description',
        'diagnosis_description_ph' => 'Describe the clinical diagnosis',
        'clinical_conduct'         => 'Clinical conduct',
        'clinical_conduct_ph'      => 'Describe the conduct and patient guidance',
        'follow_up_days'           => 'Follow-up in',
        'days'                     => 'days',

        // Documents
        'documentations'        => 'Documents',
        'new_documentation'     => 'New Document',
        'doc_type'              => 'Type',
        'doc_title'             => 'Title',
        'doc_title_ph'          => 'Document title',
        'doc_date'              => 'Date',
        'doc_actions'           => 'Actions',
        'doc_content'           => 'Content',
        'doc_content_ph'        => 'Document content…',
        'no_documentations'     => 'No documents found.',
        'select_template'       => 'Document template',
        'save_documentation'    => 'Save Document',
        'exam_hub_title'        => 'Exam Reports',
        'exam_hub_help'         => 'Select an exam to generate its report. Content will be pre-populated from the medical record data.',
        'exam_template_missing' => 'No template registered for this exam at the active clinic. Register it in Settings → Document Templates.',

        // Files
        'upload_files' => 'Attach files',
        'drop_files'   => 'Drag and drop files here or',
        'browse'       => 'Browse',

        // General
        'select'          => 'Select…',
        'cancel'          => 'Cancel',
        'close'           => 'Close',
        'not_informed'    => 'Not informed',
        'yes'             => 'Yes',
        'no'              => 'No',
        'offcanvas_title' => 'Medical Record Details',
        'covenant'        => 'Insurance',
        'skin'            => 'Skin type',
        'iris'            => 'Iris type',
        'card_number'     => 'Card number',
        'visual_exam'     => 'Visual Exam',
        'refraction'      => 'Refraction',
        'av_correction'   => 'VA with correction & prescription',
        'biomicro_fundo'  => 'Biomicroscopy & Fundoscopy',
        'observations'    => 'Observations',
        'extra_fields'    => 'HPI / Diagnosis / Conduct (expand)',
        'doc_bar_label'   => 'Prescriptions / Reports / Save',
        'save_first'      => 'Saves the record and opens this action right after',
        'print_preview'   => 'Document preview',
        'open_in_new_tab' => 'Open in a new tab',

        // Lens prescription modes (smart_oftal templates 1..4 parity)
        'lens_prescription'         => 'Lens Prescription',
        'lens_prescription_dynamic' => 'Dynamic',
        'lens_prescription_static'  => 'Static',
        'lens_prescription_presdyn' => 'Presbyopia + Dynamic',
        'lens_prescription_presbyo' => 'Presbyopia',

        // F9 — Client-side validation
        'client_errors_title'    => 'Please fix the fields before saving',
        'server_errors_title'    => 'Errors found while saving',
        'save_first_then_action' => 'Saves the record and opens this action automatically',

        // Action guards — Swal client-side
        'doctor_required_title'     => 'Physician required',
        'doctor_required_for_issue' => 'Please select the responsible physician before issuing a document.',
        'complaint_required_title'  => 'Chief complaint required',
        'complaint_required_text'   => 'Please fill in the chief complaint before issuing documents or adding attachments.',

        // Server-side validation messages (FormRequest messages())
        'doctor_required_validation' => 'Please select the responsible physician before saving the medical record.',
        'doctor_exists_validation'   => 'The selected physician does not belong to the active entity.',

        // F9 — Field labels for client-side validation messages
        'field_doctor'        => 'Physician',
        'field_complaint'     => 'Chief complaint',
        'field_pachymetry_od' => 'OD Pachymetry',
        'field_pachymetry_oe' => 'OS Pachymetry',
        'field_follow_up'     => 'Follow-up days',

        // F7 — Medical certificates (modal builder)
        'attendance_certificate_title' => 'Attendance Certificate',
        'medical_certificate_title'    => 'Medical Certificate',
        'certificate_obs_label'        => 'Additional notes (optional)',
        'certificate_obs_ph'           => 'Free text appended at the end of the certificate…',
        'certificate_emit'             => 'Issue Certificate',
        'medical_cert_days_label'      => 'Days of leave',
        'medical_cert_days_help'       => 'Written form is generated automatically.',
        'medical_cert_days_preview'    => 'Preview',
        'medical_cert_date_label'      => 'Certificate date',
        'medical_cert_date_today'      => 'Today',

        // F6 — Procedure solicitation (modal builder)
        'procedure_title'            => 'Procedure Request',
        'procedure_search_label'     => 'Add procedure',
        'procedure_search_ph'        => 'Type at least 2 characters (e.g. phaco, mapping…)',
        'procedure_type_label'       => 'Type',
        'procedure_type_rotina'      => 'Routine',
        'procedure_type_urgencia'    => 'Urgent',
        'procedure_type_controle'    => 'Control',
        'procedure_type_comparativo' => 'Comparative',
        'procedure_indication_label' => 'Add indication',
        'procedure_indication_ph'    => 'Type at least 2 characters (e.g. glaucoma, cataract…)',
        'procedure_selected'         => 'Solicitation items',
        'procedure_content'          => 'Solicitation content',
        'procedure_emit'             => 'Issue Request',
        'procedure_max_reached'      => 'Item limit reached. Please print this request before adding more.',

        // F8 — Cataract prescription (parametrised modal)
        'cataract_title'         => 'Cataract Prescription',
        'cataract_eye_label'     => 'Eye to be operated',
        'cataract_eye_right'     => 'Right Eye (OD)',
        'cataract_eye_left'      => 'Left Eye (OS)',
        'cataract_eye_both'      => 'Both Eyes (OU)',
        'cataract_template'      => 'Prescription template',
        'cataract_template_pre'  => 'Pre-operative',
        'cataract_template_pos'  => 'Post-operative',
        'cataract_template_inst' => 'Surgical instructions',
        'cataract_date'          => 'Surgery date',
        'cataract_hour'          => 'Surgery time',
        'cataract_emit'          => 'Issue Prescription',
        'cataract_help_inst'     => 'Date and time appear in the Surgical Instructions document.',

        // Clinical alerts
        'alert_self'   => 'Patient',
        'alert_family' => 'Family',

        // PDF — i18n keys
        'birth'        => 'Birth date',
        'date'         => 'Date',
        'issued_at'    => 'Issued on',
        'generated_at' => 'Generated on',
        'signed_at'    => 'Electronically signed on',
    ],

    'documentation' => [
        'types' => [
            'prescription' => 'Prescription',
            'procedure'    => 'Procedure',
            'certificate'  => 'Certificate',
            'referral'     => 'Referral',
            'report'       => 'Report',
        ],
    ],

    'profile' => [
        'photo'               => 'Profile Photo',
        'change_password'     => 'Change Password',
        'current_password'    => 'Current Password',
        'new_password'        => 'New Password',
        'confirm_password'    => 'Confirm Password',
        'update_password'     => 'Update Password',
        'saved'               => 'Profile updated successfully.',
        'password_updated'    => 'Password changed successfully.',
        'unverified_email'    => 'Email not verified',
        'resend_verification' => 'Resend verification email',
        'verification_sent'   => 'A new verification link has been sent to your email.',
        'photo_hint'          => 'JPG, PNG or WEBP. Max 2 MB.',
    ],

    'partners' => [
        // Partner portal
        'subtitle'        => 'Manage partners, leads and commissions',
        'account_created' => 'Partner registered successfully. Access email sent.',
        'portal_title'    => 'Partner Portal',
        'my_leads'        => 'My Leads',
        'my_commissions'  => 'My Commissions',

        // CRUD / actions
        'title'            => 'Partners',
        'new'              => 'New Partner',
        'new_lead'         => 'New Lead',
        'edit'             => 'Edit Partner',
        'register_partner' => 'Register partner',
        'save_changes'     => 'Save changes',
        'register_now'     => 'Register now',
        'pay'              => 'Pay',

        // Empty states
        'empty'          => 'No partners registered.',
        'no_leads'       => 'No leads registered.',
        'no_commissions' => 'No commissions generated.',

        // Tabs / sections
        'leads'       => 'Leads',
        'commissions' => 'Commissions',

        // KPIs
        'active_partners'     => 'Active Partners',
        'total_leads'         => 'Leads (total)',
        'pending_commissions' => 'Pending Commissions',
        'paid_commissions'    => 'Paid Commissions (total)',
        'lead_funnel'         => 'Lead Funnel',
        'recent_commissions'  => 'Recent Commissions',

        // Form fields
        'name'                => 'Name',
        'email'               => 'Email',
        'type'                => 'Type',
        'commission_rate'     => 'Commission Rate',
        'document'            => 'CNPJ/CPF',
        'status'              => 'Status',
        'notes'               => 'Notes',
        'select_placeholder'  => 'Select...',
        'advance_placeholder' => '— Advance —',

        // Sidebar info labels
        'commission_label' => 'Commission',
        'document_label'   => 'CNPJ/CPF',
        'utm_token'        => 'UTM Token',
        'registered_at'    => 'Registered at',

        // Status values
        'status_active'    => 'Active',
        'status_inactive'  => 'Inactive',
        'status_suspended' => 'Suspended',

        // Table headers
        'col_name'        => 'Name',
        'col_type'        => 'Type',
        'col_leads'       => 'Leads',
        'col_commissions' => 'Commissions',
        'col_pending'     => 'Pending',
        'col_paid'        => 'Paid',
        'col_actions'     => 'Actions',
        'col_partner'     => 'Partner',
        'col_clinic'      => 'Clinic',
        'col_value'       => 'Amount',
        'col_status'      => 'Status',
        'col_due'         => 'Due Date',
        'col_city'        => 'City',
        'col_advance'     => 'Advance',
        'col_date'        => 'Date',
        'col_rate'        => 'Rate',
        'col_period'      => 'Period',
    ],
];
